...
void LinkedList::append(int data)
{
    ...
    {
        tail->next = current; 
        current->prev = tail; //repair both links
    }
...
void LinkedList::prepend(int data)
{
    ...
    else
    {
        ...
        head->prev = current;   // fix backward link
    }
...
bool LinkedList::remove(int data)
{
    ...
    while (current != nullptr)          //loop til exceeding the tail.
    {
        if (current->data == data)      // found data...
        {
            ...
            {
                current->next->prev = current->prev;  // disconnect 'current', repair backward link
            }
        ...
        current = current->next;    // move backwards to previous node
    }
...
bool LinkedList::tailRemove(int data)
{
    ...
    while (current != nullptr)          //loop til exceeding the head.
    {
        if (current->data == data)      // found data...
        {
            if (current == head)
        ...
        // didn't find 'data', move to next node.
        current = current->prev;    // move forward to next node
    }
    return false; // escaped the loop without success, return failure
}
